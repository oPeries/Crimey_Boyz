...
class researcher_model extends CI_Model {
    ...
    public function insert_user($data){   
        if (isset($_POST['reg_user'])) {
            ...
            $email = $this->db->escape_str($this->input->post('username'));
          ...
          if (count($errors) == 0) {
            ...
            $sql = "INSERT INTO users (name, username, email, password, institution, field, researcher) VALUES (?, ?, ?, ?, ?, ?, TRUE)";
            $dbResult = $this->db->query($sql, array($this->input->post('name'), $this->input->post('username'), $this->input->post('email'), $password, $this->input->post('institution'), $this->input->post('field')));
